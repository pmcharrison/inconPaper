...
which can be installed using `install.packages()`:
`tidyverse`, `boot`, `memoise`, 
`purrrlyr`, `glue`, `broom`, `zeallot`,
`withr`, `jsonlite`,
`plyr`, `cowplot`, `AICcmodavg`,
`margins`,
`prediction`,
`cocor`, and
`R.utils`.

They also depend on some music-specific packages,
which can be installed at the following links:
